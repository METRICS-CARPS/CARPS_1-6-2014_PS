...
pilotNames <- "Tysen Dauer" # insert the pilot's name here e.g., "Tom Hardwicke".  If there are multiple cpilots enter both names in a character string e.g., "Tom Hardwicke, Bob Dylan"
copilotNames <- "Michele Nuijten, Erik Santoro, Jaclyn Schwartz" # # insert the co-pilot's name here e.g., "Michael Frank". If there are multiple co-pilots enter both names in a character string e.g., "Tom Hardwicke, Bob Dylan"
...
```{r}
data_tidy <- data %>%
  gather("nr_words", "perc_overlap", onegrams, twograms, threegrams) %>%
  gather("SAT_type", "SAT_score", SATreading, SATmath, SATwriting, SATcombined) %>%
  
# it becomes quite difficult to make the scores on the factual/conceptual questions tidy. The code below is a first attempt to do this, but it still needs work.
  
  # data %>%
  # gather("item_type", "score", objectiveZ, openZ, openandobjectiveZ, rawobjective, rawopen) %>%
  # mutate(score_type = ifelse(grepl("z", item_type, ignore.case = TRUE), "z", "raw")) %>%
  # mutate(item_type = ifelse(grepl("openandobjective", item_type, ignore.case = TRUE), "openandobjective", ifelse("open", item_type, ignore.case = TRUE), "open", "objective"))
  # spread(...)
```
